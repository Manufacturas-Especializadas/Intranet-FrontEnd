...
const Manufactura = () => {
  ...
  const getBlobContent = async () => {
    try {
      const response = await blogContentService.blogContent("Manufactura");
    ...
    if (sections.length === 0) {
      return (
        <div className="text-center py-20">
          <p className="text-lg text-gray-500">No hay secciones para mostrar</p>
        </div>
      );
    }
  ...
  return (
    <>
      ...
      <main className="max-w-full mx-auto space-y-2">{renderContent()}</main>
